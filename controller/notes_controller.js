...
const submit_note = async (req, res) => {
    const { new_post, email, tag } = req.body;
    if (new_post&& email &&tag!==null) {
        console.log('this is data : ');
        console.log(new_post, email, tag);

        try {
            const queryText = `
            INSERT INTO users_notes (email, post ,tags) 
            VALUES ($1, $2 ,$3) 
        `;
            const queryValues = [email, new_post, tag];
